...
#include <random>
int GenerateRandomInt(std::default_random_engine &rng, int min_value,
                      int max_value) {
...
Clipper2Lib::Paths64 GenerateRandomPaths(std::default_random_engine &rng,
                                         int min_path_count,
                                         int max_complexity) {
  std::uniform_int_distribution<int> first_point_coordinate_distribution(
      -max_complexity, max_complexity * 2);
  std::uniform_int_distribution<int> difference_to_previous_point_distribution(
      -5, 5);
  ...
  for (int path = 0; path < path_count; ++path) {
    ...
    const int path_length = GenerateRandomInt(
        rng, min_point_count, std::max(min_point_count, max_complexity));
    auto &result_path = result[path];
    result_path.reserve(path_length);
    for (int point = 0; point < path_length; ++point) {
      if (result_path.empty()) {
        result_path.emplace_back(first_point_coordinate_distribution(rng),
...
TEST(Clipper2Tests, TestRandomPaths) {
...
#endif
  {
    ...
    const Clipper2Lib::ClipType ct =
        static_cast<Clipper2Lib::ClipType>(GenerateRandomInt(rng, 0, 4));
    const Clipper2Lib::FillRule fr =
        static_cast<Clipper2Lib::FillRule>(GenerateRandomInt(rng, 0, 3));
    ...
    const int64_t area_polytree =
        static_cast<int64_t>(Area(solution_polytree_paths));
    const int64_t count_polytree =
        solution_polytree_paths.size() + solution_polytree_open.size();
