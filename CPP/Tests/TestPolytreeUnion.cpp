...
TEST(Clipper2Tests, TestPolytreeUnion) {
  ...
  if (IsPositive(subject[0]))
    clipper.Execute(ClipType::Union, FillRule::Positive, solution, open_paths);
  else {
    // because clipping ops normally return Positive solutions
    clipper.ReverseSolution(true);
    clipper.Execute(ClipType::Union, FillRule::Negative, solution, open_paths);
  }
...
TEST(Clipper2Tests, TestPolytreeUnion2) { // #987
  ...
  subject.push_back(
      MakePath({3900, -1024, 6145, -1024, 6145, 1024, 3900, 1024}));
  subject.push_back(
      MakePath({5884, 1024, 5662, 1024, 5662, -1024, 5884, -1024}));
...
TEST(Clipper2Tests, TestPolytreeUnion3) {
  ...
  subject.push_back(MakePath(
      {-120927680, 590077597, -120919386, 590077307, -120919432, 590077309,
       -120919451, 590077309, -120919455, 590077310, -120099297, 590048669,
       -120928004, 590077608, -120902794, 590076728, -120919444, 590077309,
       -120919450, 590077309, -120919842, 590077323, -120922852, 590077428,
       -120902452, 590076716, -120902455, 590076716, -120912590, 590077070,
       11914491,   249689797}));
