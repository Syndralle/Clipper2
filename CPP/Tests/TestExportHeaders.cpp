...
TEST(Clipper2Tests, ExportHeader64) {
  ...
  // Normally clipper.export.h will be compiled into a DLL/so so it can be
  // called by non C++ applications. If CreateCPaths64 was an exported function
  // and it was called by a non C++ application, it would crash that
  // application.
  CPaths64 c_subj = CreateCPathsFromPathsT(subj);
  CPaths64 c_clip = CreateCPathsFromPathsT(clip);
  BooleanOp64(Intersection, EvenOdd, c_subj, c_subj_open, c_clip, c_sol,
              c_sol_open);
...
TEST(Clipper2Tests, ExportHeaderD) {
  ...
  BooleanOpD(Intersection, EvenOdd, c_subj, c_subj_open, c_clip, c_sol,
             c_sol_open);
  solution = ConvertCPathsToPathsT(c_sol);
  // clean up !!!
  delete[] c_subj;
...
TEST(Clipper2Tests, ExportHeaderTree64) {
  ...
  BooleanOp_PolyTree64(Intersection, EvenOdd, c_subj, c_subj_open, c_clip,
                       c_sol_tree, c_sol_open);
  PolyTree64 sol_tree;
