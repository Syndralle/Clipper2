...
typedef std::function<PointInPolygonResult(const Point64 &, const Path64 &)>
    PipFunction;
...
inline PointInPolygonResult PIP1(const Point64 &pt, const Path64 &polygon) {
  int val = 0;
  typename Path64::const_iterator cbegin = polygon.cbegin(), first = cbegin,
                                  curr, prev;
...
inline PointInPolygonResult PIP2(const Point64 &pt, const Path64 &polygon) {
  ...
  if (prev->y == pt.y) {
    if (pt == *prev)
      return PointInPolygonResult::IsOn;
    if ((curr->y == pt.y) &&
        ((curr->x == pt.x) || ((pt.x > prev->x) == (pt.x < curr->x))))
      return PointInPolygonResult::IsOn;
  ...
  return (val % 2) ? PointInPolygonResult::IsInside
...
static void DoErrorTest1_internal(const Path64 &pts_of_int,
                                  const Paths64 &paths, PipFunction pip_func,
                                  PointInPolygonResult expected) {
  Path64 error_points;
  ...
  if (error_points.size()) {
    ...
    std::cout << "(" << error_points[high_error] << ")."
...
static void DoErrorTest2(int index) {
  ...
  if (errors.size()) {
    ...
    std::cout << errors[high_error] << "." << SetConsoleTextColor(reset)
              << std::endl;
  } else
    std::cout << " No errors found." << std::endl;
}
...
int main(int argc, char **argv) {
  ...
  // 1a. use const paths (PolytreeHoleOwner2.txt) with changing points of
  // interest
  ...
  std::cout << std::endl
            << SetConsoleTextColor(cyan_bold) << "Benchmarking ..."
            << SetConsoleTextColor(reset) << std::endl;
  std::cout
      << "Note: function performance varies depending on the proportion of "
         "edges"
      << std::endl
      << "that intersect with an imaginary horizontal line passing through the"
      << std::endl
  ...
  std::cout << "A single elliptical path (" << width << " x " << height << ")"
            << std::endl
  ...
  std::cout << "A random self-intersecting polygon (" << width << " x "
            << height << ")" << std::endl
            << "Edge count =  " << count << ". " << std::endl
            << "Point (" << mp << ")" << std::endl
            << std::endl;
  ...
  for (size_t i = 0; i < pipResults[0].size(); ++i) {
    if ((pipResults[0][i] == pipResults[1][i]) &&
        (pipResults[0][i] == pipResults[2][i]))
      continue;

    if (pipResults[0][i] != pipResults[1][i])
      std::cout << "PIP2 returned the " << SetConsoleTextColor(red_bold)
                << "wrong " << SetConsoleTextColor(reset)
                << "result:" << std::endl;
    if (pipResults[0][i] != pipResults[2][i])
      std::cout << "PIP3 returned the " << SetConsoleTextColor(red_bold)
                << "wrong " << SetConsoleTextColor(reset)
                << "result:" << std::endl;

    std::cout << "Problematic PIP path saved to - " << bad_filename << i
              << ".txt" << std::endl;
