...
//////////////////////////////////////////////////////////////////////////////////////
// SetConsoleTextColor: a simple class to adjust Console Text Colors (Windows &
// Linux)
//////////////////////////////////////////////////////////////////////////////////////
...
public:
  ...
  bool is_negative() const { return (hi < 0); }
...
static inline int64_t muldiv(Int128 lhs, int64_t rhs, Int128 divisor) {
  ...
  // generate a result that fits into a 64bit integer.
  int64_t sign =
      (lhs.is_negative() != divisor.is_negative()) != (rhs < 0) ? -2 : 2;
  if (lhs.is_negative())
    lhs.negate();
...
typedef std::function<bool(const Point64 &, const Point64 &, const Point64 &,
                           const Point64 &, Point64 &)>
    GipFunction;
...
static bool GIP_Current(const Point64 &ln1a, const Point64 &ln1b,
  ...
  if (det == 0.0)
    return false;
  double t =
      ((double)(ln1a.x - ln2a.x) * dy2 - (double)(ln1a.y - ln2a.y) * dx2) / det;
  if (t <= 0.0)
    ip = ln1a;
  else if (t >= 1.0)
    ip = ln1b;
...
static bool GIP_Func_F(const Point64 &ln1a, const Point64 &ln1b,
  ...
  double ln0c = (ln1dy * (double)(ln1a.x - originx)) +
                (ln1dx * (double)(ln1a.y - originy));
  double ln1c = (ln2dy * (double)(ln2a.x - originx)) +
                (ln2dx * (double)(ln2a.y - originy));
...
static bool GIP_128(const Point64 &ln1a, const Point64 &ln1b,
  ...
  Int128 t_num =
      multiply(ln1a.x - ln2a.x, dy2) - multiply(ln1a.y - ln2a.y, dx2);
...
double GetSineFrom3Points(const Point64 &a, const Point64 &b,
                          const Point64 &c) {
...
public:
  ...
  TestRecord(const Point64 &intersect_pt, const Point64 &p1, const Point64 &p2,
  ...
  for (int current_pow10 = 12; current_pow10 <= 18; ++current_pow10) {
    ...
    if (first_pass) {
      ...
      std::cout << std::endl
                << std::endl
                << SetConsoleTextColor(green_bold)
                << "Compare function accuracy ..." << SetConsoleTextColor(reset)
                << std::endl
                << "and show how it deteriorates when using very large "
                   "coordinate ranges."
                << std::endl
                << "Distance error is the distance between the calculated and "
                   "actual intersection points."
                << std::endl
                << "(The largest errors will occur whenever the segments are "
                   "close to collinear.)"
                << std::endl;
    } else {
      for (int i = 0; i < number_of_test_functions; ++i) {
        // although we're not benchmarking, we still need to collect the
        // calculated intersect points of each TestRecord for each participating
        // function. (In first_pass above, benchmark::RunSpecifiedBenchmarks()
    ...
    for (int i = 0; i < number_of_test_functions; ++i) {
      ...
      std::cout << std::fixed << GetGipFuncName(i)
                << ": average distance error = " << std::setprecision(2)
                << avg_dists[i] << "; largest dist. = " << std::setprecision(0)
                << worst_dists[i] << std::endl;
    }
    tests.clear();
  }
  return 0;
