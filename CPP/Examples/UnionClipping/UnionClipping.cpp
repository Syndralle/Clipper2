...
#ifdef _DEBUG
  // std::string s;
  // std::cout  << std::endl << "All done. Press Enter to continue" <<
  // std::endl; std::getline(std::cin, s);
#endif
  return 0;
}
...
void DoDiamonds() {
  ...
  for (int i = 0; i < h / size; i += 2) {
    ...
    shape =
        TranslatePath(shape, static_cast<int64_t>(-w / size) * size, size * 2);
  }
...
void DoSquares() {
  ...
  // SaveTest("squares.txt", false, &subjects, nullptr, nullptr, 0, 0,
  // ClipType::Union, FillRule::NonZero);
...
void DoCircles() {
  ...
  for (int j = 0; j < 550; ++j) {
    subjects.push_back(
        TranslatePath(shape, rand() % (w - 50), rand() % (h - 50)));
  }
...
void DoTriangles() {
  ...
  for (int i = 0; i < h / size / 2; ++i) {
    ...
    tri1 =
        TranslatePath(tri1, static_cast<int64_t>(-w / size) * size, size * 2);
    tri2 =
        TranslatePath(tri2, static_cast<int64_t>(-w / size) * size, size * 2);
  }
