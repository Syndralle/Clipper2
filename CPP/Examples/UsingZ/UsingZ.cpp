...
public:
  ...
  void myZCBD(const PointD &e1bot, const PointD &e1top, const PointD &e2bot,
...
static uint32_t ByteToRainbowColor(uint8_t b) {
  ...
  case 0:
    b2 = (b - 0) * 6;
    return 0xFFFF0000 | (b2 << 8); // 0xFFFF0000 -> 0xFFFFFF00 (red..yellow)
    // 43..85
  case 1:
    b2 = (85 - b) * 6;
    return 0xFF00FF00 | (b2 << 16); // 0xFFFFFF00 -> 0xFF00FF00 (yellow..lime)
    // 86..128
  case 2:
    b2 = (b - 86) * 6;
    return 0xFF00FF00 | b2; // 0xFF00FF00 -> 0xFF00FFFF (lime..aqua)
    // 129..171
  case 3:
    b2 = (171 - b) * 6;
    return 0xFF0000FF | (b2 << 8); // 0xFF00FFFF -> 0xFF0000FF (aqua..blue)
    // 172..214
  case 4:
    b2 = (b - 172) * 6;
    return 0xFF0000FF | (b2 << 16); // 0xFF0000FF -> 0xFFFF00FF (blue..fuschia)
    // 215..255
  default:
    b2 = (255 - b) * 6;
    return 0xFFFF0000 | b2; // 0xFF0000FF -> 0xFFFF00FF (fuschia..red)
  }
}

static void DisplayAsSvg(const string &filename, const Paths64 *subject,
...
static void DisplayAsSvg(const string &filename, const PathsD *subject,
                         const PathsD *clip, const PathsD *solution,
  ...
  if (solution) {
    if (multi_color) {
      ...
      for (const PathD &path : *solution) {
        ...
        uint8_t d = (path.size() == 3)
                        ? (path[0].z + path[1].z + path[2].z) / 3.0
                        : 128;
        svg.AddPath(path, false, FillRule::NonZero, ByteToRainbowColor(d),
                    0x80808080, 0.8, false);
      }
#else
...
void Test1_64() {
  ...
  c64.SetZCallback(std::bind(&MyClass::myZCB, mc, std::placeholders::_1,
  ...
  if (solution.size() > 0) {
    ...
    for (const Point64 &pt : solution[0])
      if (pt.z == 1) {
        ellipses.push_back(
            Ellipse(Rect64(pt.x - r, pt.y - r, pt.x + r, pt.y + r), 11));
      }
  }
...
void Test1_Double() {
  ...
  c.SetZCallback(std::bind(&MyClass::myZCBD, mc, std::placeholders::_1,
                           std::placeholders::_2, std::placeholders::_3,
                           std::placeholders::_4, std::placeholders::_5));
