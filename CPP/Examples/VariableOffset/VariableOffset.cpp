...
void test1() {
  ...
  ClipperOffset co;
  co.SetDeltaCallback([delta](const Path64 &path, const PathD &path_norms,
...
void test3() {
  ...
  co.SetDeltaCallback([radius](const Path64 &path, const PathD &path_norms,
                               size_t curr_idx, size_t prev_idx) {
    // when multiplying the x & y of edge unit normal vectors, the value will be
    // largest (0.5) when edges are at 45 deg. and least (-0.5) at negative 45
    // deg.
...
void test4() {
  ...
  Paths64 solution;
  Paths64 subject = {
      Ellipse(ScaleRect<int64_t, int64_t>(Rect64(10, 10, 50, 50), scale))};

  ClipperOffset co;
  co.AddPaths(subject, JoinType::Round, EndType::Round);
  co.Execute(
      [scale](const Path64 &path, const PathD &path_norms, size_t curr_idx,
              size_t prev_idx) {
        // double vertex_sin_a = CrossProduct(path_norms[curr_idx],
        // path_norms[prev_idx]); double vertex_cos_a =
        // DotProduct(path_norms[curr_idx], path_norms[prev_idx]); double
        // vertex_angle = std::atan2(vertex_sin_a, vertex_cos_a); double
        // edge_angle = std::atan2(path_norms[curr_idx].y,
        // path_norms[curr_idx].x);
...
void test5() {
  ...
  co.Execute([](const Path64 &path, const PathD &path_norms, size_t curr_idx,
                size_t prev_idx) { return double(curr_idx * curr_idx + 10); },
             solution);
