...
static PathsD MakeRandomPolys(unsigned polyCount, unsigned vertCnt, int width,
                              int height) {
  PathsD result;
...
static void DisplaySolution(const string filename, const PathsD &sol,
                            bool multicolor) {
  SvgWriter svg;
...
int main() {
  //////////////////////////////////////////////////////////////////////
  Test t = Test::sample_1; // Test::random; //Test::single_test;
                           // //Test::range_test; //Test::clipper; //
  ...
  case Test::range_test: {
    for (int i = minRange; i <= maxRange; ++i) {
      ...
      if (tr != TriangulateResult::success)
  ...
  case Test::random: {
    ...
    // The Triangulate function below only accepts **non-intersecting** paths,
    // so ...
  ...
  case Test::sample_3: {
    PathsD subject = {MakePathD(
        {44.1, 114.0, 11.9, 114.0, 13.9, 72.9, 0.0,  59.9, 0.0,  53.2, 12.2,
         41.0, 16.8,  41.0, 10.1,  37.2, 8.0,  19.9, 11.3, 6.7,  20.0, 1.9,
         20.0, 0.0,   34.6, 0.0,   44.9, 6.8,  48.0, 29.1, 45.7, 37.3, 39.2,
         41.0, 43.8,  41.0, 56.0,  53.2, 56.0, 59.9, 42.0, 72.9})};
    Triangulate(subject, 0, sol, true);
