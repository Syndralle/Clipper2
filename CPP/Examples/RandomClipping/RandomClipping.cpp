...
int GenerateRandomNumber(std::default_random_engine &rng, int min_value,
                         int max_value) {
...
Paths64 GenerateRandomPaths(std::default_random_engine &rng, int path_count,
                            int edge_count) {
  if (!path_count)
    return Paths64();
...
void RandomTest(int path_count) {
  ...
  subj = GenerateRandomPaths(rng, path_count,
  ...
  // SaveTest("random.txt", false, &subj, &subj_open, &clip, 0, 0, cliptype,
  // fillrule); int64_t area, cnt; std::ifstream ifs("random2.txt");
  ...
  SvgAddCaption(svg,
                ClipTypeToString(cliptype) + ", " + FillRuleToString(fillrule),
                20, 20);
