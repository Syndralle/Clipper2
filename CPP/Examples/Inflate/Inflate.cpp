...
static void DisplayAsSvgImage(const std::string &caption, FillRule fillrule,
                              const PathsD *subject, const PathsD *clip,
                              const PathsD *solution) {
...
static void DoRabbit() {
  ...
  DisplayAsSvgImage("rabbit_offset", FillRule::EvenOdd, nullptr, nullptr,
                    &solution);
}
...
static void DoSimpleShapes() {
  ...
  SvgWriter svg2;
  op1.push_back(MakePathD(
      {80, 60, 20, 20, 180, 20, 180, 70, 25, 150, 20, 180, 180, 180}));
  ...
  // precision, we have to scale manually. (InflatePaths does this scaling
  // internally)
  ClipperOffset co;
  p = ScalePaths<double, double>(p, scale, err);
  // AddPaths - paths must be int64_t paths
  co.AddPath(TransformPath<int64_t, double>(p[0]), JoinType::Bevel,
             EndType::Joined);
  co.AddPath(TransformPath<int64_t, double>(p[1]), JoinType::Square,
             EndType::Joined);
  co.AddPath(TransformPath<int64_t, double>(p[2]), JoinType::Round,
             EndType::Joined);
  Paths64 sol64;
