/*******************************************************************************
 * Author    :  Angus Johnson  * Date      :  21 February 2026
 *                              * Release   :  BETA RELEASE
 *                               * Website   :  https://www.angusj.com
 *                                * Copyright :  Angus Johnson 2010-2026
 *                                 * Purpose   :  Constrained Delaunay
 * Triangulation                              * License   :
...
static bool LeftTurning(const Point64 &p1, const Point64 &p2,
                        const Point64 &p3) {
  return CrossProductSign(p1, p2, p3) < 0;
}

static bool RightTurning(const Point64 &p1, const Point64 &p2,
                         const Point64 &p3) {
  return CrossProductSign(p1, p2, p3) > 0;
}
...
static size_t Next(size_t &idx, size_t len) { return (idx + 1) % len; }

static Edge *FindLinkingEdge(const Vertex2 *vert1, const Vertex2 *vert2,
...
static double ShortestDistFromSegment(const Point64 &pt, const Point64 &segPt1,
                                      const Point64 &segPt2) {
...
void Delaunay::ForceLegal(Edge *edge) {
  ...
  double ictResult =
      InCircleTest(vertA->pt, edge->vL->pt, edge->vR->pt, vertB->pt);
  ...
  for (int i = 1; i < 3; ++i) {
    ...
    // since each edge has its own triangleA and triangleB, we have to be
    // careful to update the correct one ...
  ...
  for (size_t i1 = 0; i1 < allEdges.size(); ++i1) {
    ...
    for (size_t i2 = i1 + 1; i2 < allEdges.size(); ++i2) {
      ...
      if (e2->vT->pt.y < e1->vB->pt.y && e2->vB->pt.y > e1->vT->pt.y &&
          (SegsIntersect(e2->vL->pt, e2->vR->pt, e1->vL->pt, e1->vR->pt) ==
           IntersectKind::intersect)) {
...
void Delaunay::MergeDupOrCollinearVertices() {
  ...
  for (auto vIter2 = allVertices.begin() + 1; vIter2 != allVertices.end();
       ++vIter2) {
...
Edge *Delaunay::CreateInnerLocMinLooseEdge(Vertex2 *vAbove) {
  ...
  if (xBest < xAbove) {
    while (e) {
      if (e->vR->pt.x > xBest && e->vL->pt.x < xAbove && e->vB->pt.y > yAbove &&
          e->vT->pt.y < yBest &&
          (SegsIntersect(e->vB->pt, e->vT->pt, vBest->pt, vAbove->pt) ==
           IntersectKind::intersect)) {
...
Edge *Delaunay::HorizontalBetween(Vertex2 *v1, Vertex2 *v2) {
  ...
  while (res) {
    if (res->vL->pt.y == y && res->vR->pt.y == y && res->vL->pt.x >= l &&
        res->vR->pt.x <= r && (res->vL->pt.x != l || res->vL->pt.x != r))
      break;
    res = res->nextE;
  }
  return res;
...
void Delaunay::DoTriangulateLeft(Edge *edge, Vertex2 *pivot, int64_t minY) {
  ...
  if (!eX) {
    ...
    if (vAlt->pt.y == v->pt.y && v->pt.y == minY && HorizontalBetween(vAlt, v))
      return;
...
void Delaunay::AddPath(const Path64 &path) {
  ...
  for (;;) {
    ...
    if (!lowermostVertex || vPrev->pt.y > lowermostVertex->pt.y ||
  ...
  i = vert_cnt;
  if (len < 3 ||
      (len == 3 && // or just a very tiny triangle
...
TriangulateResult Triangulate(const Paths64 &pp, Paths64 &solution,
                              bool useDelaunay) {
...
TriangulateResult Triangulate(const PathsD &pp, int decPlaces, PathsD &solution,
                              bool useDelaunay) {
  int ec = 0;
  double scale;
  TriangulateResult result;
