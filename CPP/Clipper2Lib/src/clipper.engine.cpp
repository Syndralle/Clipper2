...
#define nearbyint(a)                                                           \
  _mm_cvtsd_si64(_mm_set_sd(a)) /* Note: expression type is (int64_t) */
#endif
...
void ClipperBase::DoSplitOp(OutRec *outrec, OutPt *splitOp) {
  ...
  Point64 ip;
  GetLineIntersectPt(prevOp->pt, splitOp->pt, splitOp->next->pt, nextNextOp->pt,
                     ip);
...
bool ClipperBase::CheckSplitOwner(OutRec *outrec, OutRecList *splits) {
  // nb: use indexing (not an iterator) in case 'splits' is modified inside this
  // loop (#1029)
