/*******************************************************************************
 * Author    :  Angus Johnson  * Date      :  12 October 2025
 *                              * Website   :  https://www.angusj.com
 *                               * Copyright :  Angus Johnson 2010-2025
 *                                * Purpose   :  Core Clipper Library structures
 * and functions                   * License   :
...
template <typename T> struct Point {
  ...
  template <typename T2> Point(const T2 x_, const T2 y_, const z_type z_ = 0) {
    Init(x_, y_);
    z = z_;
  }

  template <typename T2> explicit Point(const Point<T2> &p) {
    Init(p.x, p.y, p.z);
  }

  template <typename T2> explicit Point(const Point<T2> &p, z_type z_) {
    Init(p.x, p.y, z_);
  }
  ...
  template <typename T2> inline void Init(const T2 x_ = 0, const T2 y_ = 0) {
    if constexpr (std::is_integral_v<T> && is_round_invocable<T2>::value &&
                  !std::is_integral_v<T2>) {
      x = static_cast<T>(std::round(x_));
      y = static_cast<T>(std::round(y_));
  ...
  template <typename T2> Point(const T2 x_, const T2 y_) { Init(x_, y_); }

  template <typename T2> explicit Point(const Point<T2> &p) { Init(p.x, p.y); }
...
template <typename T> Paths<T> &operator<<(Paths<T> &polys, const Path<T> &p) {
...
template <typename T> Rect<T> GetBounds(const Path<T> &path) {
...
template <typename T> Rect<T> GetBounds(const Paths<T> &paths) {
  T xmin = (std::numeric_limits<T>::max)();
  T ymin = (std::numeric_limits<T>::max)();
  T xmax = std::numeric_limits<T>::lowest();
  T ymax = std::numeric_limits<T>::lowest();
...
template <typename T, typename T2> Rect<T> GetBounds(const Path<T2> &path) {
  T xmin = (std::numeric_limits<T>::max)();
  T ymin = (std::numeric_limits<T>::max)();
  T xmax = std::numeric_limits<T>::lowest();
  T ymax = std::numeric_limits<T>::lowest();
...
template <typename T, typename T2> Rect<T> GetBounds(const Paths<T2> &paths) {
  T xmin = (std::numeric_limits<T>::max)();
  T ymin = (std::numeric_limits<T>::max)();
  T xmax = std::numeric_limits<T>::lowest();
  T ymax = std::numeric_limits<T>::lowest();
...
template <typename T> inline double Sqr(T val) {
...
inline UInt128Struct MultiplyUInt64(uint64_t a, uint64_t b) // #834, #835
{
  ...
  return {uint64_t(lo(x3) << 32 | lo(x1)),
          uint64_t(hi(a) * hi(b) + hi(x2) + hi(x3))};
}
...
template <typename T> inline double Area(const Path<T> &path) {
...
template <typename T> inline double Area(const Paths<T> &paths) {
  double a = 0.0;
...
template <typename T> inline bool IsPositive(const Path<T> &poly) {
...
template <typename T> inline int GetSign(const T &val) {
  if (!val)
    return 0;
  return (val > 0) ? 1 : -1;
}
