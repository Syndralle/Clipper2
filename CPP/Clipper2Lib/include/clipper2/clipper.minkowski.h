/*******************************************************************************
 * Author    :  Angus Johnson  * Date      :  1 November 2023
...
namespace detail {
inline Paths64 Minkowski(const Path64 &pattern, const Path64 &path, bool isSum,
                         bool isClosed) {
...
inline Paths64 MinkowskiSum(const Path64 &pattern, const Path64 &path,
                            bool isClosed) {
  return detail::Union(detail::Minkowski(pattern, path, true, isClosed),
                       FillRule::NonZero);
}

inline PathsD MinkowskiSum(const PathD &pattern, const PathD &path,
                           bool isClosed, int decimalPlaces = 2) {
  int error_code = 0;
...
inline Paths64 MinkowskiDiff(const Path64 &pattern, const Path64 &path,
                             bool isClosed) {
  return detail::Union(detail::Minkowski(pattern, path, false, isClosed),
                       FillRule::NonZero);
}

inline PathsD MinkowskiDiff(const PathD &pattern, const PathD &path,
                            bool isClosed, int decimalPlaces = 2) {
  int error_code = 0;
  double scale = pow(10, decimalPlaces);
  Path64 pat64 = ScalePath<int64_t, double>(pattern, scale, error_code);
