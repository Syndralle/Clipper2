/*******************************************************************************
 * Author    :  Angus Johnson  * Date      :  5 March 2025
 *                              * Website   :  https://www.angusj.com
 *                               * Copyright :  Angus Johnson 2010-2025
 *                                * Purpose   :  This module provides a simple
 * interface to the Clipper Library  * License   :
...
inline Paths64 Intersect(const Paths64 &subjects, const Paths64 &clips,
                         FillRule fillrule) {
  return BooleanOp(ClipType::Intersection, fillrule, subjects, clips);
}

inline PathsD Intersect(const PathsD &subjects, const PathsD &clips,
                        FillRule fillrule, int decimal_prec = 2) {
  return BooleanOp(ClipType::Intersection, fillrule, subjects, clips,
                   decimal_prec);
}

inline Paths64 Union(const Paths64 &subjects, const Paths64 &clips,
                     FillRule fillrule) {
  return BooleanOp(ClipType::Union, fillrule, subjects, clips);
}

inline PathsD Union(const PathsD &subjects, const PathsD &clips,
                    FillRule fillrule, int decimal_prec = 2) {
...
inline Paths64 Difference(const Paths64 &subjects, const Paths64 &clips,
                          FillRule fillrule) {
  return BooleanOp(ClipType::Difference, fillrule, subjects, clips);
}

inline PathsD Difference(const PathsD &subjects, const PathsD &clips,
                         FillRule fillrule, int decimal_prec = 2) {
  return BooleanOp(ClipType::Difference, fillrule, subjects, clips,
                   decimal_prec);
}

inline Paths64 Xor(const Paths64 &subjects, const Paths64 &clips,
                   FillRule fillrule) {
  return BooleanOp(ClipType::Xor, fillrule, subjects, clips);
}

inline PathsD Xor(const PathsD &subjects, const PathsD &clips,
                  FillRule fillrule, int decimal_prec = 2) {
  return BooleanOp(ClipType::Xor, fillrule, subjects, clips, decimal_prec);
}

inline Paths64 InflatePaths(const Paths64 &paths, double delta, JoinType jt,
                            EndType et, double miter_limit = 2.0,
...
inline PathsD InflatePaths(const PathsD &paths, double delta, JoinType jt,
                           EndType et, double miter_limit = 2.0,
  ...
  clip_offset.AddPaths(ScalePaths<int64_t, double>(paths, scale, error_code),
                       jt, et);
  if (error_code)
    return PathsD();
  Paths64 solution;
...
inline Path<T> TranslatePath(const Path<T> &path, T dx, T dy) {
  ...
  std::transform(
      path.begin(), path.end(), back_inserter(result),
...
inline Paths<T> TranslatePaths(const Paths<T> &paths, T dx, T dy) {
  ...
  std::transform(
      paths.begin(), paths.end(), back_inserter(result),
...
inline PathsD RectClip(const RectD &rect, const PathsD &paths,
                       int precision = 2) {
  if (rect.IsEmpty() || paths.empty())
    return PathsD();
  int error_code = 0;
...
inline PathsD RectClip(const RectD &rect, const PathD &path,
                       int precision = 2) {
...
inline PathsD RectClipLines(const RectD &rect, const PathsD &lines,
                            int precision = 2) {
  if (rect.IsEmpty() || lines.empty())
    return PathsD();
  int error_code = 0;
...
inline PathsD RectClipLines(const RectD &rect, const PathD &line,
                            int precision = 2) {
...
static void OutlinePolyPath(std::ostream &os, size_t idx, bool isHole,
                            size_t count, const std::string &preamble) {
...
inline constexpr void MakePathGeneric(const T an_array, size_t array_size,
                                      std::vector<U> &result) {
...
inline PathD TrimCollinear(const PathD &path, int precision,
...
template <typename T>
inline bool NearCollinear(const Point<T> &pt1, const Point<T> &pt2,
                          const Point<T> &pt3, double sin_sqrd_min_angle_rads) {
...
template <typename T>
inline Path<T> Ellipse(const Point<T> &center, double radiusX,
                       double radiusY = 0, size_t steps = 0) {
  if (radiusX <= 0)
    return Path<T>();
  if (radiusY <= 0)
    radiusY = radiusX;
...
template <typename T>
inline Path<T> SimplifyPath(const Path<T> &path, double epsilon,
  ...
  if (isClosedPath) {
    ...
    distSqr[high] =
        PerpendicDistFromLineSqrd(path[high], path[0], path[high - 1]);
  } else {
    distSqr[0] = MAX_DBL;
...
template <typename T>
inline Paths<T> SimplifyPaths(const Paths<T> &paths, double epsilon,
                              bool isClosedPath = true) {
  Paths<T> result;
  result.reserve(paths.size());
...
template <typename T>
inline void RDP(const Path<T> path, std::size_t begin, std::size_t end,
                double epsSqrd, std::vector<bool> &flags) {
