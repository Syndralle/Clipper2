/*******************************************************************************
 * Author    :  Angus Johnson  * Date      :  6 December 2025
...
inline int32_t RandomColor() { return rand() | (rand() << 15) | 0xFF000000; }
...
inline void SvgAddCaption(SvgWriter &svg, const std::string &caption, int x,
                          int y) {
...
inline void SvgAddSubject(SvgWriter &svg, const PathsD &path,
  ...
  if (svg.Fill_Rule() == FillRule::Positive ||
    ...
    svg.AddPaths(tmp, false, fillrule, subj_brush_clr, subj_stroke_clr, 0.8,
                 false);
  } else
    svg.AddPaths(path, false, fillrule, subj_brush_clr, subj_stroke_clr, 0.8,
                 false);
}
...
inline void SvgAddOpenSubject(SvgWriter &svg, const PathsD &path,
                              FillRule fillrule = FillRule::EvenOdd,
                              bool is_joined = false) {
  if (is_joined)
    svg.AddPaths(path, false, fillrule, subj_brush_clr, subj_stroke_clr, 1.3,
                 false);
  else
    svg.AddPaths(path, true, fillrule, 0x0, subj_stroke_clr, 1.3, false);
}

inline void SvgAddOpenSubject(SvgWriter &svg, const Paths64 &path,
                              FillRule fillrule = FillRule::EvenOdd,
                              bool is_joined = false) {
  svg.AddPaths(TransformPaths<double, int64_t>(path), !is_joined, fillrule, 0x0,
               0xCCB3B3DA, 1.3, false);
}

inline void SvgAddClip(SvgWriter &svg, const PathsD &path, FillRule fillrule) {
  svg.AddPaths(path, false, fillrule, clip_brush_clr, clip_stroke_clr, 0.8,
               false);
}
...
inline void SvgAddClip(SvgWriter &svg, const Paths64 &paths,
                       FillRule fillrule) {
  PathsD tmp = TransformPaths<double, int64_t>(paths);
  svg.AddPaths(tmp, false, fillrule, clip_brush_clr, clip_stroke_clr, 0.8,
               false);
}

inline void SvgAddSolution(SvgWriter &svg, const PathsD &path,
                           FillRule fillrule, bool show_coords) {
  svg.AddPaths(path, false, fillrule, solution_brush_clr, 0x80666666, 1.0,
               show_coords);
}

// SvgAddRCSolution - random color for each individual polygon (eg when
// triangulating)
inline void SvgAddRCSolution(SvgWriter &svg, const PathsD &paths,
                             FillRule fillrule, bool show_coords) {
  for (const PathD &path : paths)
    svg.AddPath(path, false, fillrule, RandomColor(), 0xFF999999, 1.0,
                show_coords);
}

inline void SvgAddSolution(SvgWriter &svg, const Paths64 &path,
                           FillRule fillrule, bool show_coords) {
  svg.AddPaths(TransformPaths<double, int64_t>(path), false, fillrule,
               solution_brush_clr, 0xFF003300, 1.0, show_coords);
}

// SvgAddRCSolution - random color for each individual polygon (eg when
// triangulating)
inline void SvgAddRCSolution(SvgWriter &svg, const Paths64 &paths,
                             FillRule fillrule, bool show_coords) {
  for (const Path64 &path : paths) {
    svg.AddPath(TransformPath<double, int64_t>(path), false, fillrule,
                RandomColor(), 0xFF999999, 1.0, show_coords);
  }
}
...
inline void SvgAddOpenSolution(SvgWriter &svg, const Paths64 &path,
                               ...
                               bool is_joined = false) {
  svg.AddPaths(TransformPaths<double, int64_t>(path), !is_joined, fillrule, 0x0,
               0xFF006600, 1.8, show_coords);
}

inline void SvgSaveToFile(SvgWriter &svg, const std::string &filename,
